...
create unique index if not exists crm_catalog_serial_number_unique on public.crm_catalog_records ((payload->>'numero_serie'))
  where entity = 'serials' and not deleted;
create unique index if not exists crm_catalog_version_date_unique on public.crm_catalog_records ((payload->>'versao'),(payload->>'data_versao'))
