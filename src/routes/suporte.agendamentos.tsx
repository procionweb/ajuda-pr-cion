...
function SupportAppointmentsPage() {
  ...
  const operators = useMemo(
    () =>
      [...new Set(events.map((event) => event.responsible || event.operator).filter(Boolean))].sort(),
    [events],
  );
    ...
    <AppShell fullWidth>
      ...
      <section className="mb-5 grid gap-3 xl:grid-cols-[190px_minmax(240px,1fr)_180px_200px_155px_155px_auto]">
        ...
        <label className="relative">
          ...
          <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Pesquisar chamado, título ou cliente" className="h-10 pl-9" />
        </label>
        ...
        <Input type="date" value={from} onChange={(event) => setFrom(event.target.value)} className="h-10" aria-label="Data inicial" />
        <Input type="date" value={to} onChange={(event) => setTo(event.target.value)} className="h-10" aria-label="Data final" />
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[760px] table-fixed text-left text-[13px] text-foreground">
            ...
            <tbody className="divide-y">
              {rows.length === 0 ? (
                <tr><td colSpan={7} className="h-52 text-center text-muted-foreground">Nenhum agendamento de suporte encontrado.</td></tr>
              ) : rows.map((event) => (
                <AppointmentRow key={String(event.id)} event={event} onOpen={() => setSelectedEvent(event)} />
              ))}
            </tbody>
          </table>
        </div>
...
const selectClass = "h-10 w-full rounded-md border border-input bg-background px-3 text-sm text-foreground shadow-sm outline-none focus:border-primary focus:ring-2 focus:ring-ring/25";
